...
where p.start_time >= '2026-01-28'
  and p.start_time < DATE_ADD('2026-01-28', INTERVAL 1 DAY);
```
...
join channels as c
 on p.channel_id = c.id
where c.name = 'ドラマ'
  and p.start_time >= '2026-01-28'
  and p.start_time < DATE_ADD('2026-01-28', INTERVAL 7 DAY);
```
...
select
 ...
 round(avg(b.view_count)) as 'エピソード平均視聴数'
from broadcasts
join episodes
 on broadcasts.episode_id = e.id
join programs as p
 on e.program_id = p.id
join program_genres
 on programs.id = program_genres.program_id
join genres
 on program_genres.genre_id = genres.id
group by episodes.program_id)

select
 max(round(avg(b.view_count)))
from program_avgs
