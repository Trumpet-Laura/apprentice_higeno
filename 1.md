...
### 番組枠
```sql
INSERT INTO program_slots (id, channel_id, start_time, end_time) VALUES
-- アニメ（鬼滅の刃）
(1, 3, CURDATE() + INTERVAL 19 HOUR, CURDATE() + INTERVAL 19 HOUR + INTERVAL 30 MINUTE),
(2, 4, CURDATE() + INTERVAL 22 HOUR, CURDATE() + INTERVAL 22 HOUR + INTERVAL 30 MINUTE),

-- ドラマ（半沢直樹）
(3, 1, CURDATE() + INTERVAL 21 HOUR, CURDATE() + INTERVAL 21 HOUR + INTERVAL 45 MINUTE),
(4, 2, CURDATE() + INTERVAL 23 HOUR, CURDATE() + INTERVAL 23 HOUR + INTERVAL 45 MINUTE),

-- スポーツ（サッカー名勝負集）
(5, 5, CURDATE() - INTERVAL 2 DAY, CURDATE() - INTERVAL 2 DAY + INTERVAL 1 HOUR);
```

### 放送
```sql
INSERT INTO broadcasts (id, program_slot_id, episode_id) VALUES
-- 鬼滅の刃（アニメ1・アニメ2）
(1, 1, 1), -- 第1話
(2, 2, 2), -- 第2話
...
```
### エピソード視聴数トップ3の番組タイトル、シーズン数、エピソード数、エピソードタイトル、視聴数
```sql
